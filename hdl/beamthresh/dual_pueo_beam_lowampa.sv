...
module dual_pueo_beam_lowampa #(parameter WBCLKTYPE = "PSCLK", 
        ...
        input [NCHAN:0] beamA_use,
        input [NCHAN:0] beamA_invert,
        input [NCHAN*NSAMP*NBITS-1:0] beamB_i,
        input [NCHAN:0] beamB_use,
        input [NCHAN:0] beamB_invert,
    ...
    wire [15:0] ternaryA_out[1:0];
    wire [15:0] ternaryB_out[1:0];
    ...
    generate
        ...
        for (jj=0;jj<NSAMP;jj=jj+1) begin : SV
            ...
            // beamform A
            fivebit_8way_ternary #(.ADD_CONSTANT(5'd2)) // The constant add is to correct for the -0.5 in each offset binary
                u_beamA(.clk_i(clk_i),
        ...
        for (kk=0;kk<2;kk=kk+1) begin : TERN
            wire [13:0] Ax = beamA_sq[2*kk];
            wire [13:0] Ay = beamA_sq[2*kk+1];
            wire [13:0] Az = {14{1'b0}};//(kk==2) ? {14{1'b0}} : beamA_sq[3*kk+2];
            ...
            wire [13:0] Bx = beamB_sq[2*kk];
            wire [13:0] By = beamB_sq[2*kk+1];
            wire [13:0] Bz = {14{1'b0}};//(kk==2) ? {14{1'b0}} : beamB_sq[3*kk+2];
            ternary_add_sub_prim #(.input_word_size(14),
                                   .is_signed(1'b0))
                u_ternB(.clk_i(clk_i),
                        .rst_i(1'b0),
