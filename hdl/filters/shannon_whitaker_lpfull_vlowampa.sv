...
module shannon_whitaker_lpfull_vlowampa #(parameter NBITS=12,
    ...
    generate
        ...
        for (ii=0;ii<NSAMPS;ii=ii+1) begin : S
            for (jj=0;jj<NBITS;jj=jj+1) begin : B
                ...
                assign xin[NSAMPS+ii][jj] = samp_extend[ii][jj];
            end
        end
    endgenerate    
    ...
    generate
        ...
        for (i=0;i<NSAMPS;i=i+1) begin : DLY
            ...
            srlvec #(.NBITS(NBITS)) u_delay(.clk(clk_i),.ce(1'b1),.a(4),.din(samp_store),.dout(samp_srldelay));
            srlvec #(.NBITS(NBITS)) u_delay2(.clk(clk_i),.ce(1'b1),.a(4),.din(samp_store2),.dout(samp_srldelay2));
            always @(posedge clk_i) begin : DLYFF
                samp_extend[i] <= xin[i];
                samp_store <= xin[NSAMPS+i];
                samp_store2 <= samp_store;
                samp_delay <= samp_srldelay;
                samp_delay2 <= samp_srldelay2;
            end
            assign xin_store[i] = samp_store;
            assign xin_store[NSAMPS+i] = samp_store2;
            assign xin_delay[i] = samp_delay;
            assign xin_delay[NSAMPS+i] = samp_delay2;
