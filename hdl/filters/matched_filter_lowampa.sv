...
//           0, -4, -4,  1,  4,  1, -2, -1,
//           1,  1 ]
module matched_filter_lowampa #(parameter NBITS=12,
                        parameter NSAMPS=4)(
        input aclk,
    ...
    // input data delayed
    reg [NBITS*NSAMPS-1:0] data_delayed_1 = {NBITS*NSAMPS{1'b0}}; //1x delayed
    reg [NBITS*NSAMPS-1:0] data_delayed_2 = {NBITS*NSAMPS{1'b0}}; //2x delayed
    reg [NBITS*NSAMPS-1:0] data_delayed_3 = {NBITS*NSAMPS{1'b0}}; //3x delayed
    // input data doubly-delayed (4x on half clock rate)
    reg [NBITS*NSAMPS-1:0] data_delayed_4 = {NBITS*NSAMPS{1'b0}}; //4x delayed
    reg [NBITS*NSAMPS-1:0] data_delayed_5 = {NBITS*NSAMPS{1'b0}}; //4x delayed
    wire [NBITS*2*NSAMPS-1:0] datax8;
    wire [NBITS*2*NSAMPS-1:0] datax8_zminus8;
    wire [NBITS*2*NSAMPS-1:0] datax8_zminus16;

    assign datax8 = {data_i,data_delayed_1};
    assign datax8_zminus8 = {data_delayed_2,data_delayed_3};
    assign datax8_zminus16 = {data_delayed_4,data_delayed_5};

    // vectorized
    wire [NBITS-1:0] x[NSAMPS*2-1:0];
    wire [NBITS-1:0] x_zminus8[NSAMPS*2-1:0];
    wire [NBITS-1:0] x_zminus16[NSAMPS*2-1:0];
    wire [NBITS-1:0] x_zminus1[NSAMPS*2-1:0];    
    wire [NBITS-1:0] x_zminus15[NSAMPS*2-1:0];
        
    // 1+z^-1
    wire [NBITS:0] aux_a[NSAMPS*2-1:0];
    wire [NBITS:0] aux_a_zminus8[NSAMPS*2-1:0];
    // mixed to create zminus 5 for the b terms
    wire [NBITS:0] aux_a_zminus5[NSAMPS*2-1:0];

    // n.b. b term inputs have range -4096 to 4094 so they have range -8190 to +8190
    wire [NBITS+1:0] aux_b[NSAMPS*2-1:0];
    wire [NBITS+1:0] aux_b_zminus8[NSAMPS*2-1:0];
    wire [NBITS+1:0] aux_b_zminus16[NSAMPS*2-1:0];

    wire [NBITS:0] aux_c[NSAMPS*2-1:0];
    wire [NBITS:0] aux_c_zminus8[NSAMPS*2-1:0];
    wire [NBITS:0] aux_c_zminus16[NSAMPS*2-1:0];
    
    always @(posedge aclk) begin
        data_delayed_1 <= data_i;
        data_delayed_2 <= data_delayed_1;
        data_delayed_3 <= data_delayed_2;
        data_delayed_4 <= data_delayed_3;
        data_delayed_5 <= data_delayed_4;
    end
    
    generate
        genvar i;
        for (i=0;i<NSAMPS;i=i+1) begin : NL
            assign x[i] = datax8[NBITS*i +: NBITS];
            assign x[NSAMPS+i] = datax8[NBITS*(i+NSAMPS) +: NBITS];
            assign x_zminus8[i] = datax8_zminus8[NBITS*i +: NBITS];
            assign x_zminus8[NSAMPS+i] = datax8_zminus8[NBITS*(NSAMPS+i) +: NBITS];
            assign x_zminus16[i] = datax8_zminus16[NBITS*i +: NBITS];
            assign x_zminus16[NSAMPS+i] = datax8_zminus16[NBITS*(NSAMPS+i) +: NBITS];
            assign x_zminus1[i] = (i==0) ? x_zminus8[2*NSAMPS+i-1] : x[i-1];
            assign x_zminus1[NSAMPS+i] = x[NSAMPS+i-1];
            assign x_zminus15[i] = (i<(2*NSAMPS-1)) ? x_zminus16[i+1] : x_zminus8[i+1-2*NSAMPS];
            assign x_zminus15[NSAMPS+i] =(i<(NSAMPS-1)) ? x_zminus16[NSAMPS+i+1] : x_zminus8[i+1-NSAMPS]; //NSAMPS-NSAMPS cancels
            assign aux_a_zminus5[i] = aux_a_zminus8[2*NSAMPS+i-5];
            assign aux_a_zminus5[NSAMPS+i] = (NSAMPS+i < 5) ? aux_a_zminus8[2*NSAMPS+i-5] : aux_a[NSAMPS+i-5];
           
            reg [NBITS:0] aux_a_sum_future_1 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_a_sum = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_a_sum_delayed_1 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_a_sum_delayed_2 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_a_sum_delayed_3 = {NBITS+1{1'b0}};
            reg [NBITS+1:0] aux_b_sum_future_1 = {NBITS+1{1'b0}};
            reg [NBITS+1:0] aux_b_sum = {NBITS+2{1'b0}};
            reg [NBITS+1:0] aux_b_sum_delayed_1 = {NBITS+2{1'b0}};
            reg [NBITS+1:0] aux_b_sum_delayed_2 = {NBITS+2{1'b0}};
            reg [NBITS+1:0] aux_b_sum_delayed_3 = {NBITS+2{1'b0}};
            reg [NBITS+1:0] aux_b_sum_delayed_4 = {NBITS+2{1'b0}};
            reg [NBITS+1:0] aux_b_sum_delayed_5 = {NBITS+2{1'b0}};
            reg [NBITS:0] aux_c_sum_future_1 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum_delayed_1 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum_delayed_2 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum_delayed_3 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum_delayed_4 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum_delayed_5 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum_delayed_6 = {NBITS+1{1'b0}};
            reg [NBITS:0] aux_c_sum_delayed_7 = {NBITS+1{1'b0}};
                        
            assign aux_a[i] = aux_a_sum_delayed_1;
            assign aux_a[NSAMPS+i] = aux_a_sum;
            assign aux_a_zminus8[i] = aux_a_sum_delayed_3;
            assign aux_a_zminus8[NSAMPS+i] = aux_a_sum_delayed_2;
            assign aux_b[i] = aux_b_sum_delayed_1;
            assign aux_b[NSAMPS+i] = aux_b_sum;
            assign aux_b_zminus8[i] = aux_b_sum_delayed_3;
            assign aux_b_zminus8[NSAMPS+i] = aux_b_sum_delayed_2;
            assign aux_b_zminus16[i] = aux_b_sum_delayed_5;
            assign aux_b_zminus16[NSAMPS+i] = aux_b_sum_delayed_4;
            // aux_c is base delayed by 1.
            assign aux_c[i] = aux_c_sum_delayed_3;
            assign aux_c[NSAMPS+i] = aux_c_sum_delayed_2;
            assign aux_c_zminus8[i] = aux_c_sum_delayed_5;
            assign aux_c_zminus8[NSAMPS+i] = aux_c_sum_delayed_4;
            assign aux_c_zminus16[i] = aux_c_sum_delayed_7;
            assign aux_c_zminus16[NSAMPS+i] = aux_c_sum_delayed_6;
            ...
            // 2: z^24 z^24 z^24
            if (i == 3) begin : T1DL
                reg [NBITS+1:0] Ty_delayed_1 = {NBITS+1{1'b0}};
                reg [NBITS+1:0] Ty_delayed_2 = {NBITS+1{1'b0}};
                always @(posedge aclk) begin : LG
                    Ty_delayed_1 <= Ty;
                    Ty_delayed_2 <= Ty_delayed_1;
                end
                assign T_1 = Ty_delayed_2;
            end else begin
                assign T_1 = Ty;
            end
            // 0, 1, and 2 just add direct, everyone else needs Tz delayed.
            if (i > 2) begin : T2DL
                reg [NBITS+1:0] Tz_delayed_1 = {NBITS+1{1'b0}};
                reg [NBITS+1:0] Tz_delayed_2 = {NBITS+1{1'b0}};
                always @(posedge aclk) begin : LG
                    Tz_delayed_1 <= Tz;
                    Tz_delayed_2 <= Tz_delayed_1;
                end
                assign T_2 = Tz_delayed_2;
            end else begin
                assign T_2 = Tz;
            end
            ...
            wire [NBITS+2:0] U_1;
            if (i < 7) begin : HD2
                reg [NBITS+2:0] TC_delayed_1 = {NBITS+3{1'b0}};
                reg [NBITS+2:0] TC_delayed_2 = {NBITS+3{1'b0}};
                always @(posedge aclk) begin : LG
                    TC_delayed_1 <= TC;
                    TC_delayed_2 <= TC_delayed_1;
                end
                assign U_1 = TC_delayed_2;
            end else begin : TL2
                assign U_1 = TC;            
            end
	    ...
	    //
	    // 4-samp means delay needs to be 2(old_delay)
	    // since address is n-2, we need 2n-2, or 2(n-2)+2
	    ...
	    //8-samp to 4-samp helper registers
	    reg [NBITS+2:0] Tx_zplus4 = {NBITS+3{1'b0}};
	    reg [NBITS+1:0] Ty_zplus4 = {NBITS+2{1'b0}};
	    reg [NBITS+1:0] Tz_zplus4 = {NBITS+2{1'b0}};
	    reg [NBITS+4:0] T_sum_zplus4 = {NBITS+5{1'b0}};
	    reg [NBITS+3:0] TB_zplus4 = {NBITS+4{1'b0}};
	    reg [NBITS+2:0] TC_zplus4 = {NBITS+3{1'b0}};
	    reg [NBITS+4:0] U_sum_zplus4 = {NBITS+5{1'b0}};
	    reg [NBITS+5:0] M_sum_zplus4 = {NBITS+4{1'b0}};
	    reg [NBITS-1:0] M_sat_and_scale_zplus4 = {NBITS{1'b0}};
...
//                aux_a_sum <= aux_a_sum_future_1; 
		aux_a_sum_delayed_1 <= aux_a_sum;
		aux_a_sum_delayed_2 <= aux_a_sum_delayed_1;
		aux_a_sum_delayed_3 <= aux_a_sum_delayed_2;
                //aux_b_sum <= aux_b_sum_future_1; 
		aux_b_sum_delayed_1 <= aux_b_sum;
		aux_b_sum_delayed_2 <= aux_b_sum_delayed_1;
		aux_b_sum_delayed_3 <= aux_b_sum_delayed_2;
		aux_b_sum_delayed_4 <= aux_b_sum_delayed_3;
		aux_b_sum_delayed_5 <= aux_b_sum_delayed_4;
//                aux_c_sum <= aux_c_sum_future_1; 
		aux_c_sum_delayed_1 <= aux_c_sum;
		aux_c_sum_delayed_2 <= aux_c_sum_delayed_1;
		aux_c_sum_delayed_3 <= aux_c_sum_delayed_2;
		aux_c_sum_delayed_4 <= aux_c_sum_delayed_3;
		aux_c_sum_delayed_5 <= aux_c_sum_delayed_4;
		aux_c_sum_delayed_6 <= aux_c_sum_delayed_5;
		aux_c_sum_delayed_7 <= aux_c_sum_delayed_6;
                
                Tx <= Tx_zplus4;
                Tx_zplus4 <= Two_Tx_0_SE - Tx_2_SE - Two_Tx_1_SE;
                
                Ty <= Ty_zplus4;
                Ty_zplus4  <= Ty_0_SE+ Ty_2_SE - Ty_1_SE ;
                
		        Tz <= Tz_zplus4;
                Tz_zplus4 <= Tz_0_SE - Tz_1_SE + Tz_2_SE;
                
                T_sum <= T_sum_zplus4;
		T_sum_zplus4 <= T_0_SE + Four_T_1_SE + T_2_SE;
                
                TB <= TB_zplus4;
                TB_zplus4 <= TB_2_SE - TB_0_SE - TB_1_SE;
                
                TC <= TC_zplus4;
                TC_zplus4 <= Two_TC_2_SE - TC_0_SE - TC_1_SE;

	        U_2 <= U_2_dly;	       
	       
                U_sum <= U_sum_zplus4;
		U_sum_zplus4 <= U_0_SE + U_1_SE + U_2_SE;
                
                T_delayed_sum <= T_delayed;
                
                M_sum <= M_sum_zplus4;
		M_sum_zplus4 <= M_0_SE + M_1_SE;
                
                if (saturation) begin
                    // this is always the same
                    M_sat_and_scale_zplus4[NBITS-1] <= M_sum[4+NBITS-1];
                    // this is always the opposite: so it's either 011111111111
                    //                                          or 100000000000
                    M_sat_and_scale_zplus4[NBITS-2:0] <= ~M_sum[4+NBITS-1];
                end else begin
                    // FIGURE OUT ROUNDING!!!
                    M_sat_and_scale <= M_sum[4 +: NBITS];
                end
//		M_sat_and_scale <= M_sat_and_scale_zplus4;
            end                             
